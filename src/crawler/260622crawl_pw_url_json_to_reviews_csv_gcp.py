...
async def scrape_restaurants_from_json(
    ...
    print(
        f"Loaded {len(restaurants)} restaurants from {input_path}. "
    ...
    for index, restaurant in enumerate(restaurants, start=1):
        ...
        output_path = output_dir / f"reviews_{label}.csv"
        print("")
        print("=" * 72)
        print(
...
def parse_args():
    ...
    parser.add_argument(
        "--input-json",
        required=not bool(os.getenv("INPUT_JSON", "").strip()),
        default=os.getenv("INPUT_JSON", "").strip(),
        help=(
    ...
    parser.add_argument(
        ...
        help=(
            "Batch CSV output directory. Defaults to ./output when "
            "--input-json is used."
        ),
    )
